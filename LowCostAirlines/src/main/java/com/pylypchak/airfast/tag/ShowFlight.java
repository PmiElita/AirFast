...
public class ShowFlight extends SimpleTagSupport {
		...
		if (flights != null) {
			...
			try {
				for (Flight f : flights) {
					...
					out.println("<p>"+Messager.getMessage("flight.from", locale)+" ");
					out.println("<span class=\"foundedFrom\">"
							+ service.getCountryCityAirportNamesByAirportId(f
									.getFromAirportId()) + "</span></p>");
					out.println("<p>"+Messager.getMessage("flight.to", locale)+" <span class=\"foundedTo\">"
							+ service.getCountryCityAirportNamesByAirportId(f
					...
					out.println("</div>");
					out.println("<form action=\"ticketForm\" method=\"get\">");
					out.println("<input type=\"text\" name=\"flightId\" value=\""
							+ f.getId() + "\" hidden>");
					out.println("<button type=\"submit\" class=\"buyTicket\"><img src=\"resources/images/buy.svg\" alt=\"\"></button>");
					out.println("</form>");
					out.println("</div>");
				}
			} catch (Exception e) {
